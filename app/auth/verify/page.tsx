...
function AuthVerifyContent() {
  ...
  useEffect(() => {
    queueMicrotask(() => {
      setLocale((getStoredLocale() ?? defaultLocale) as Locale);
    });
  }, []);

  const t = getTranslations(locale).authVerify;
  ...
  useEffect(() => {
    ...
    verify({ token, locale });
  }, [token, verify, dispatch, router, locale]);
  ...
  }, [isSuccess, accessToken, dispatch, router, locale]);
